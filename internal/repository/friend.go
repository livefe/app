...
func (r *friendRepository) UpdateFriendStatus(id uint, status int) error {
	...
	// 更新当前记录状态
	if err := tx.Model(&model.Friend{}).Where("id = ?", id).Update("status", status).Error; err != nil {
		tx.Rollback()
		return err
	}

	// 更新对应的另一条记录状态
	if err := tx.Model(&model.Friend{}).Where(
...
func (r *friendRepository) DeleteFriend(userID, targetID uint) error {
	...
	// 删除第一条记录（用户视角）
	if err := tx.Where("user_id = ? AND target_id = ?", userID, targetID).Delete(&model.Friend{}).Error; err != nil {
		tx.Rollback()
		return err
	}

	// 删除第二条记录（好友视角）
	if err := tx.Where("user_id = ? AND target_id = ?", targetID, userID).Delete(&model.Friend{}).Error; err != nil {
		tx.Rollback()
		return err
	}
